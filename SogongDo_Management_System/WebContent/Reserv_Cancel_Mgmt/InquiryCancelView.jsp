...
<h3>숙박 예약 취소 관리</h3>
        ...
        <div>
        	<button class="bt register">예약취소 등록하기</button>
           	<button class="bt delete">예약취소 삭제하기</button>
			<button class="bt update">예약취소 수정하기</button>
        </div>
	...
	window.addEventListener("load",function(){
		...
		registerbtn.addEventListener("click",()=>{
			//새로운 창 뜨도록 하기
			window.open("/SogongDo_Management_System/Reserv_Cancel_Mgmt/RegisterCancelView.jsp","예약 취소 정보 등록","width=500, height=400, left=300, top=300");
		})
		...
		deletebtn.addEventListener("click",()=>{
			...
			if(ids.length==0){
				...
				//삭제컨트롤러에 전송~!
				var ox = confirm("예약 취소 정보를 삭제하시겠습니까?");
				if(ox){
					var url = "/SogongDo_Management_System/Reservation/Cancel/delete?";
					url += "cnt="+ids.length+"&";
		...
		updatebtn.addEventListener("click",()=>{
			...
			if(cnt==0){
				...
				window.open("/SogongDo_Management_System/Reservation/Cancel/update/view?id="+up,"예약 취소 정보 수정","width=500, height=400, left=300, top=300");
			}else{
				alert('하나만 선택');
			}
		})
